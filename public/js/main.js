...
const signup = async (btn) => {
  ...
  if (name === '' || email === '' || pass === '' || cpass === '') {
    ...
    btn.innerHTML =
      '<span class="spinner-border text-warning spinner-border" role="status" aria-hidden="true"></span>'
...
const operTask = async (action, e) => {
  ...
  if (action === 'fetch') {
    ...
    if (
      resFetchData.message !== 'noData' &&
      resFetchData.message !== 'notLogged'
    ) {
    ...
    if (title.value === '' || desc.value === '') {
      ...
      e.target.innerHTML =
        '<span class="spinner-border text-primary spinner-border" role="status" aria-hidden="true"></span>'
      const userTaskData = {
        action: action,
        title: title.value,
        desc: desc.value
  ...
  if (action === 'edit') {
    ...
    if (upTaskData.message === 'Updation successful') {
      document.getElementById(
        taskNumber
      ).parentElement.children[0].children[0].innerHTML =
        `${taskNumber + 1}.&nbsp;&nbsp;${upTitle}`
